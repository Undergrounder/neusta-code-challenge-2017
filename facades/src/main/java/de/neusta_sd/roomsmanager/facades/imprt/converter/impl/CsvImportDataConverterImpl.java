...
public class CsvImportDataConverterImpl implements CsvImportDataConverter {
    ...
    @Override
    public ImportService.ImportData convert(final ImportCsvParser.CsvImportData source) throws ConversionException {
        final List<ImportCsvParser.CsvRoomData> sourceRoomDataList = source.getRoomDataList();
        final List<ImportService.RoomData> roomDataList = ConverterUtils.convertAll(sourceRoomDataList, getCsvRoomDataConverter());

        return ImportService.ImportData.builder()
